            ...
            <tr>
                <td style="width:250px; text-align:right;padding-right:50px">
                    <div class="nav flex-column nav-pills" id="routine" role="tablist" aria-orientation="vertical">
                        <button class="nav-link rounded-pill" 
                            :class="active_routine == r.class_name ? 'active blue-outline': ''" data-bs-toggle="pill" type="button" role="tab"
                            @click="active_routine = r.class_name "
                ...
                <td style="height:400px; overflow:auto">
                    ...
                    <div class="list-group list-group-flush" id="subroutine" v-if="subroutine">
                        <div class="list-group-item list-group-item-action " :style="active_subroutine == index ? '':  active_subroutine > index ? 'opacity:0.8' : 'opacity:0.3'"  v-for="([index, sub], i) in subroutine"  :key="i">
...
const active_routine = ref("D30")
const active_subroutine = ref(2)
...
const subroutine = computed(() => { 
    let ret = store.routines.find(sr => sr.class_name.toUpperCase() == active_routine.value )
    if (typeof ret !== "undefined")   {
        return Object.entries(ret.subroutine)
    }
    return []
